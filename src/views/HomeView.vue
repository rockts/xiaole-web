  ...
  <main class="home-view">
    ...
    <template v-else-if="home">
      ...
      <section v-if="showProfile" class="quiet-section profile-section" data-home-section="profile">
        <details>
          <summary><span><strong>让小乐更懂你</strong><small>为了让推荐更准确，还有 {{ home.profile_status.needs_confirmation_count }} 项资料需要确认。</small></span><span class="summary-action">查看</span></summary>
          <div class="profile-fields"><p v-for="field in home.profile_status.fields" :key="field.key"><span>{{ field.label }}</span><strong>{{ field.value || '待确认' }}</strong></p></div>
        </details>
      </section>
...
const showProfile = computed(() => home.value?.profile_status?.status === 'available' && home.value.profile_status.needs_confirmation_count > 0)
