...
const loadInbox = async () => { inboxFailed.value = false; inboxDegraded.value = false; try { const result = await api.getIntelligenceInbox('all'); inboxItems.value = result.items || []; inboxDegraded.value = Boolean(result.degraded) } catch { inboxItems.value = []; inboxFailed.value = true } }
...
onMounted(() => { load(); loadInbox(); confirmationStore.load() })
</script>
