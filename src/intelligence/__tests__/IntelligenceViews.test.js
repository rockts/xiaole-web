...
describe('Intelligence views', () => {
  ...
  it('does not optimistically mark read when PUT fails', async () => {
    ...
    expect(wrapper.get('[data-test="user-details"]').text()).toContain('需核实申报条件')
    expect(wrapper.get('[data-test="technical-details"]').text()).toContain('matched')
    expect(wrapper.get('[data-test="technical-details"]').text()).toContain('official_source')
    expect(wrapper.get('[data-test="technical-details"]').attributes('open')).toBeUndefined()
