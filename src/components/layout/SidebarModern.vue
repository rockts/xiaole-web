<template>
  <aside :key="isMobile ? (collapsed ? 'mobile-closed' : 'mobile-open') : 'desktop'" class="product-sidebar" :class="{ collapsed, open: isMobile && !collapsed }" aria-label="小乐导航" :aria-hidden="isMobile && collapsed" :inert="isMobile && collapsed ? '' : null">
    ...
    <div class="sidebar-body">
      ...
      <section class="recent-section" aria-labelledby="recent-title">
        ...
        <button type="button" class="view-all" data-testid="view-all-conversations" @click="navigateMobile('/conversations')">查看全部 <span>→</span></button>
      </section>

      <nav v-if="isMobile" class="drawer-secondary" aria-label="更多导航">
        <button type="button" data-testid="settings-link" @click="navigateMobile('/settings')"><span>设置</span><span>→</span></button>
        <button type="button" data-testid="advanced-link" @click="navigateMobile('/behavior')"><span>高级与诊断</span><small>行为分析、工具等</small></button>
      </nav>
    </div>
  ...
  <button v-if="isMobile && !collapsed" class="product-sidebar-backdrop" type="button" aria-label="关闭菜单" @click="closeMobile"></button>
...
const navigateMobile = (path, beforeNavigate) => { beforeNavigate?.(); const navigation = router.push(path); closeMobile(); return navigation }
const newChat = () => navigateMobile('/chat', () => chatStore.clearCurrentSession())
const loadSession = (session) => navigateMobile(`/chat/${sessionId(session)}`)
const goHome = () => isMobile.value ? navigateMobile('/home') : router.push('/home')
...
@media(max-width:768px){.product-sidebar{position:fixed;top:0;bottom:0;left:0;width:min(86vw,320px);min-width:0;max-width:320px;height:100dvh;padding-bottom:env(safe-area-inset-bottom);box-shadow:var(--shadow-lg);transform:translateX(0)}.product-sidebar.collapsed:not(.open){width:min(86vw,320px);min-width:0;transform:translateX(-110%);box-shadow:none}.sidebar-brand{min-height:60px}.collapse-button{min-width:44px;min-height:44px}.sidebar-body{padding:0 12px 12px}.new-chat-button{min-height:48px}.recent-section{padding-top:18px}.recent-row{min-height:44px}.more-button{min-width:44px;min-height:44px;opacity:1}.view-all{min-height:48px}.product-sidebar-backdrop{position:fixed;z-index:880;inset:0;display:block;border:0;background:rgba(15,23,42,.32);backdrop-filter:blur(2px)}}
.drawer-secondary button{display:flex;width:100%;min-height:48px;align-items:center;justify-content:space-between;padding:0 10px;border:0;border-radius:10px;background:transparent;color:var(--text-primary);font:inherit;text-align:left;cursor:pointer}.drawer-secondary button:hover{background:var(--bg-secondary)}
</style>
