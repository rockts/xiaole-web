  ...
  <article class="confirmation-card" data-test="confirmation-card">
    ...
    <form v-else class="replacement-form" @submit.prevent="saveReplacement">
      ...
      <div class="form-actions">
        <button v-if="hasCandidate" type="button" class="secondary" :disabled="submitting" @click="editing = false">取消</button>
        <button type="submit" data-test="save-replacement" :disabled="submitting || !canSave">{{ submitting ? '正在保存…' : '保存' }}</button>
      </div>
    </form>
